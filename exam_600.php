...
$count = 4; //共產生幾筆
for ($i = 1; $i <= $count; $i++) {
    $randval = mt_rand(1,475); //取得範圍為1~177841亂數
...
<form method="post">
<input type="button" value="A.<?php echo $ca; echo " "; echo $eca; ?>" onClick="this.form.action='exam_600.php';this.form.submit();"> 
<input type="hidden" value="<?php echo $a; ?>" name="choice" >
</form>
<form method="post">
<input type="button" value="B.<?php echo $cb; echo " "; echo $ecb; ?>" onClick="this.form.action='exam_600.php';this.form.submit();"> 
<input type="hidden" value="<?php echo $b; ?>" name="choice" >
</form>
<form method="post">
<input type="button" value="C.<?php echo $cc; echo " "; echo $ecc;?>" onClick="this.form.action='exam_600.php';this.form.submit();"> 
<input type="hidden" value="<?php echo $c; ?>" name="choice" >
</form>
<form method="post">
<input type="button" value="D.<?php echo $cd; echo " "; echo $ecd; ?>" onClick="this.form.action='exam_600.php';this.form.submit();"> 
...
if($_POST[choice] != null)
{
	if($_POST[choice] == $cor)
	{
		...
		$sql = "insert into exam_600_record (id, english, correct_time, false_time, time, user) values ( '".$cor."','".$title."', '1', '', now(), '".$username."'  )";
        $result = $mysqli -> query($sql) or die("無法執行：".mysql_error()); 

        echo "累計正確次數：";
		$sql="SELECT COUNT(*) as C1 FROM exam_600_record WHERE `id`=$cor AND `correct_time`=1";
        $result = $mysqli -> query($sql) or die("無法執行：".mysql_error());
        $row=$result -> fetch_array();
		echo $row['C1'];
		echo "次，錯誤次數：";
		$sql="SELECT COUNT(*) AS C2 FROM exam_600_record WHERE `id`=$cor AND `false_time`=1";
        $result = $mysqli -> query($sql) or die("無法執行：".mysql_error());
        $row=$result -> fetch_array();
		echo $row['C2'];
		echo "次。<br>";
	...
	else if($_POST[choice] != $cor)
	{
		...
		echo $row['C1'];
		echo "次，錯誤次數：";
		$sql="SELECT false_time AS C2 FROM exam_600_record WHERE `id`=$cor AND `false_time`=1";
        $result = $mysqli -> query($sql) or die("無法執行：".mysql_error());
        $row=$result -> fetch_array();
		echo $row['C2'];
		echo "次。";
...
$sql="select * from pydict where english = '$title'";
$result = $mysqli -> query($sql) or die("無法執行：".mysql_error());
while($row = $result -> fetch_row()){
echo "<br>- Chinese：".$row["2"]."<br><br>";
}

$sql="select * from `exam_600_record` where `correct_time`='1' group by `english`";
